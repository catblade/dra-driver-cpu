	...
	ginkgo.When("setting resource claims", func() {
		...
		ginkgo.Context("for exclusive CPU allocation", func() {
			...
			ginkgo.It("should allocate exclusive CPUs and remove from the shared pool", func(ctx context.Context) {
				...
				shrPod1 := mustCreateBestEffortPodPinned(ctx, fxt, targetNode, dracpuTesterImage)
				...
				shrPod2 := mustCreateBestEffortPodPinned(ctx, fxt, targetNode, dracpuTesterImage)
				...
				fixture.By("recreating best-effort pods to re-evaluate shared pool")
				gomega.Expect(e2epod.DeleteSync(ctx, fxt.K8SClientset, shrPod1)).To(gomega.Succeed(), "cannot delete pod %s", e2epod.Identify(shrPod1))
				gomega.Expect(e2epod.DeleteSync(ctx, fxt.K8SClientset, shrPod2)).To(gomega.Succeed(), "cannot delete pod %s", e2epod.Identify(shrPod2))
				shrPod1 = mustCreateBestEffortPodPinned(ctx, fxt, targetNode, dracpuTesterImage)
				shrPod2 = mustCreateBestEffortPodPinned(ctx, fxt, targetNode, dracpuTesterImage)
...
func mustCreateBestEffortPodPinned(ctx context.Context, fxt *fixture.Fixture, node *v1.Node, dracpuTesterImage string) *v1.Pod {
	fixture.By("creating a best-effort reference pod")
